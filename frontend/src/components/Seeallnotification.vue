<template>
    <div class="min-h-screen bg-[#FFF3EB] flex flex-col">
      ...
      <header class="bg-[#FFF3EB] w-full z-10 fixed top-0 left-0">
        <div class="mx-auto px-8 flex items-center justify-between md:h-20 w-full bg-[#fff3eb]">
      ...
      <div class="flex-grow flex p-4 gap-4 h-full mt-12">
        <div class="w-1/2 py-4 pr-4 flex-grow flex flex-col items-center">
          <div class="w-11/12 h-auto bg-white p-4 rounded-md">
            ...
            <div class="space-y-4">
              <div
                class="border-t border-gray-300 py-2"
                v-for="(mesg, index) in notification"
                :key="index"
              >
                <div class="flex items-start space-x-2">
                  <div class="w-full" @click="goto({ path: `/seeeachnotification/${mesg.id}` })">
                    <p class="text-sm text-gray-800 text-left font-medium">
                      {{ mesg.message }}
                    </p>
      ...
      methods: {
        async fetchNotifications() {
          try {
            ...
            this.notification = response.data.map(notif => ({
              id: notif.id, // Ensure to include the ID here
