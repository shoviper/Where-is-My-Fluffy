	...
	methods: {
		...
		async fetchReward() {
			try {
				...
				console.log(data);
				return data.rewardAmount; // Modify this if you want a different post's reward
			} catch (error) {
		...
		async sendMessage() {
			...
			try {
				...
				const getCurrentPost = await axios.get(
					`http://localhost:8080/posts/${this.postId}`
				);
				const postData = getCurrentPost.data;

				const getMeAgain = await axios.get(
					`http://localhost:8080/users/me`,
					{
						withCredentials: true,
					}
				);
				const usersMeData = getMeAgain.data;

				const handlePetStatusChange = await axios.put(
					`http://localhost:8080/pets/update-pet-status`,
					{
						id: postData.pet.id,
						status: 'FOUND',
					}
				);
				console.log(handlePetStatusChange.data);

				const handlePetFounderChange = await axios.put(
					`http://localhost:8080/pets/update-pet-founder`,
					{
						petId: postData.pet.id,
						founderId: usersMeData.id,
					}
				);
				console.log(handlePetFounderChange.data);
